...
class ReferenceDownloader:
    ...
    def download_file(self, url: str, dest_path: str, description: str = "文件") -> Optional[str]:
        ...
        if dest_path.exists():
            logger.info(f"✓ {description}已存在: {dest_path}")
    ...
    def download_hg38_genome(self) -> Optional[str]:
        """下载hg38参考基因组"""
        # 使用Ensembl的CRAM参考，更快速和小
        url = "http://ftp.ensembl.org/pub/release-109/fasta/homo_sapiens/dna/Homo_sapiens.GRCh38.dna.primary_assembly.fa.gz"
        dest_path = self.ref_dir / "hg38.fa"

        return self.download_file(url, dest_path, "hg38参考基因组")

    def download_hg38_gtf(self) -> Optional[str]:
        """下载hg38基因注释"""
        url = "http://ftp.ensembl.org/pub/release-109/gtf/homo_sapiens/Homo_sapiens.GRCh38.109.gtf.gz"
        dest_path = self.ref_dir / "hg38.gtf"

        return self.download_file(url, dest_path, "hg38基因注释")
